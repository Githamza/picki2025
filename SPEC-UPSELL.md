...
**Status:** IMPLEMENTED 2026-08-09 (T1–T12) — success criteria 1–5 and 7 verified by unit + e2e suites; criterion 6's kiosk idle-warning interplay needs one manual walkthrough. The `category_type` migration is **live in prod** (pushed 2026-08-09, user-approved, together with the previously unpushed `kiosk_enabled` migration); the frontend deploy is the remaining rollout step.
