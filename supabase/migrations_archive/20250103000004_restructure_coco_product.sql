-- Restructure Coco Product (ID: 135) with new 6-step configuration

-- First, delete existing steps and options for Coco product
DELETE FROM product_step_options 
WHERE step_id IN (SELECT id FROM product_steps WHERE product_id = 135);

DELETE FROM product_steps WHERE product_id = 135;
...
VALUES (135, 1, 'Taille de la box', 'Choisissez-en 1.', 'single-select', true, 1, 1);
...
FROM product_steps WHERE product_id = 135 AND display_order = 1;
...
INSERT INTO product_steps (product_id, display_order, name, description, step_type, is_required, min_selections, max_selections) 
VALUES (135, 2, 'Je choisis ma protéine', 'Choisissez-en 1.', 'single-select', true, 1, 1);

INSERT INTO product_step_options (step_id, product_id, name, description, price_adjustment, option_type, image_url, display_order, is_available)
SELECT id, NULL, 'Boeuf Grillé', 'Boeuf grillé savoureux', 1.90, 'component', 'https://example.com/beef-grilled.jpg', 1, true
FROM product_steps WHERE product_id = 135 AND display_order = 2;
...
INSERT INTO product_steps (product_id, display_order, name, description, step_type, is_required, min_selections, max_selections) 
VALUES (135, 3, 'je choisis mes extras', 'Choisissez-en 6 max.', 'multi-select', false, 0, 6);

INSERT INTO product_step_options (step_id, product_id, name, description, price_adjustment, option_type, image_url, display_order, is_available)
SELECT id, NULL, 'Poulet Frit x3', '3 pièces de poulet frit', 5.90, 'component', 'https://example.com/fried-chicken.jpg', 1, true
FROM product_steps WHERE product_id = 135 AND display_order = 3;
...
INSERT INTO product_steps (product_id, display_order, name, description, step_type, is_required, min_selections, max_selections) 
VALUES (135, 4, 'Des suppléments ?', 'Choisissez-en 5 max.', 'multi-select', false, 0, 5);

INSERT INTO product_step_options (step_id, product_id, name, description, price_adjustment, option_type, image_url, display_order, is_available)
SELECT id, NULL, 'Suppléments Viandes', 'Supplément de viandes', 4.50, 'component', 'https://example.com/meat-supplement.jpg', 1, true
FROM product_steps WHERE product_id = 135 AND display_order = 4;
...
-- Step 5: Une boisson ? (Optional, max 14) - Same as Mafé, Yassa and Antillaise
INSERT INTO product_steps (product_id, display_order, name, description, step_type, is_required, min_selections, max_selections) 
VALUES (135, 5, 'Une boisson ?', 'Choisissez-en 14 max.', 'multi-select', false, 0, 14);

-- Insert all drink options (same as previous products)
INSERT INTO product_step_options (step_id, product_id, name, description, price_adjustment, option_type, image_url, display_order, is_available)
...
FROM product_steps ps WHERE ps.product_id = 135 AND ps.display_order = 5;
...
INSERT INTO product_steps (product_id, display_order, name, description, step_type, is_required, min_selections, max_selections) 
VALUES (135, 6, 'Un dessert ?', 'Choisissez-en 1 max.', 'single-select', false, 0, 1);
...
FROM product_steps WHERE product_id = 135 AND display_order = 6; 