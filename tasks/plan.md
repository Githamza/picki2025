...
1. **RESOLVED BY DECISION (2026-08-09): dialog-first is deliberate.** `diningPreferenceGuard` stays permissive: the app rests on the home page (category grid) — the restaurant owner wants it open there — and customers who haven't chosen a dining preference pick it inside the checkout dialog (`needsPreferenceStep`). The welcome screen remains reachable via the toolbar preference button; the kiosk attract screen plays the "entry gate" role in kiosk mode.
2. **FIXED (2026-08-09):** the checkout submit button now reads "Valider la commande" (receipt icon) whenever no payment screen follows — counter vendors and kiosk mode — and keeps "Continuer vers le paiement" for online payments.
3. **FIXED (2026-08-09):** labels no longer hard-code asterisks (Material's required marker supplies one), which also resolves the label-over-value rendering; helper asserts no starred `mat-label` remains.
